...
def to_img_ecvl_yml_str(ds, train_idxs, valid_idxs, test_idxs, descr="n/a"):
    out = []
    out.append("name: ECVL dataset for UC5")
    out.append("")
    out.append(f"description: {descr}")
    out.append("")
    classes = repr(list( set([a for y in ds.labels.tolist() for a in y] ) ) )
    ...
    out.append(f"  validation: {repr(valid_idxs)}")
    out.append(f"  test: {repr(test_idxs)}")
    return "\n".join(out)
