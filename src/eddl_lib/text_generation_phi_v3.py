...
def apply_thresholds(values, thresholds):
    ...
    assert thresholds is not None, "thresholds is none"
...
def annotate(img, cnn, rnn, target_text, n_tokens, dev=False, thresholds=None):
    ...
    if thresholds is not None:
        ths = apply_thresholds(cnn_semantic, thresholds)
...
def load_thresholds(config):
    fn = join(config["exp_fld"], "thresholds_trva.tsv")
...
def main(out_fn,
    ...
    eddl.build(cnn, eddl.adam(0.01), ["softmax_cross_entropy"], ["accuracy"], eddl.CS_GPU(g=[1,0,0,0]), init_weights=False)
    eddl.set_mode(cnn, 0)
    ...
    eddl.build(rnn, eddl.adam(0.01), ["softmax_cross_entropy"], ["accuracy"], eddl.CS_GPU(g=[1,0,0,0]), init_weights=False)
